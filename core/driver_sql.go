...
func (d *stmtDriver) Init(log logger.Logger) error {
	d.log = log
	...
	if d.driver.SupportsTx() && d.driver.UseGlobalTx() {
		...
		if err != nil {
			log.Error("unable to start tx", field("error", err))
			return err
		}
...
func (d *stmtDriver) Migrate(migration *ParsedMigration, beforeFinish func(target DBTarget) error) error {
	for _, s := range migration.Stmts {
		d.log.Debug("executing statement", field("statement", s))

		started := time.Now()
		if _, err := d.target.Exec(s); err != nil {
			d.log.Error("failed executing statement", field("statement", s), field("error", err))
			d.rollback = true
			return err
		}
		end := time.Now()

		d.log.Debug("executing statement took", field("duration", end.Sub(started)))
	}
	...
	if beforeFinish != nil {
		...
		if err != nil {
			d.log.Error("beforeFinishCallback failed", field("error", err))
			d.rollback = true
			return err
		} else {
...
func (d *stmtDriver) Close() error {
	...
	if d.tx != nil {
		...
		if d.rollback {
			...
			err := d.tx.Rollback()
			if err != nil {
				d.log.Error("rollback of global tx failed", field("error", err))
			} else {
			...
			err := d.tx.Commit()
			if err != nil {
				d.log.Error("commit of global tx failed", field("error", err))
			} else {
