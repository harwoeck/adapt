	...
	tests := []struct {
		...
		{"complex multiple sources with holes and sorting", args{[]Source{
			...
			NewCodePackageSource(map[string]Hook{
				"20201115_1215_init-code":      Hook{},
				"20201115_1215_init-code-2":    Hook{},
				"20201115_1718_undo-init-code": Hook{},
			}),
		}}, []*AvailableMigration{
			{ID: "20201115_1214_init"},
...
func Test_healthCheckAppliedMigration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := healthCheckAppliedMigration(tt.args.applied, logger.MustNewStd()); (err != nil) != tt.wantErr {
...
func Test_unknownAppliedMigrations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			unknown, err := unknownAppliedMigrations(tt.args.applied, tt.args.available, true, logger.MustNewStd())
			if (err != nil) != tt.wantErr {
...
func Test_findNeededMigrations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := findNeededMigrations(tt.args.applied, tt.args.available, logger.MustNewStd()); !reflect.DeepEqual(got, tt.want) {
